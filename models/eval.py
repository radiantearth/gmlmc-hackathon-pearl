...
parser.add_argument('--batch_size', type=int, default=2, help='Batch size to use during inference.')
...
parser.add_argument('--num_classes', type=int, default=11, help='number of classes model was trained with'),

parser.add_argument('--label_transform', default='naip', help='str either naip, epa or cic to indicate how to transform labels')
...
def label_transform_naip5cls(labels):
    labels = np.array(labels).astype(np.int64)
    labels_new = np.copy(labels)
...
def labels_transform_uvm(labels):
    labels = np.array(labels).astype(np.int64)
    labels_new = np.copy(labels)
...
def main():
    ...
    if args.label_transform == "naip":
        ...
        class_names = ['no_data', 'water', 'emergent_wetlands', 'tree_canopy', 'shrubland', 'low_vegetation',
                        'barren', 'structure', 'impervious_surface', 'impervious_roads', 'weighted_avg']
    elif args.label_transform == "epa":
        label_transform = label_transforms_epa
        class_names = ['no_data', 'impervious', 'soil_barren', 'grass', 'tree/forest', 'water', 'shrub', 'woody_wetlands',
                    'emergent_wetlands', 'agriculture', 'orchard', 'weighted_avg']
    elif args.label_transform == 'cic':
        label_transform = label_transform_cic
        class_names = ['Structures', 'Impervious Surface', 'Water', 'Grassland/Pairie', 'Tree Canopy', 'Turff', 'Barren/Rock', 'Irregated']

    elif args.label_transform == 'naip_5cls':
        label_transform = label_transform_naip5cls
        class_names = ['water/wetland', 'tree', 'barren', 'low veg', 'built enviornment']
        ...
        class_names = ['tree', 'grass', 'bare soil', 'water', 'buildings', 'roads', 'other impervious']
    else:
    ...
    for image_idx in range(len(image_fns)):
        ...
        # remove no data vals
        gt_cleaned = np.delete(gt_f, np.where((gt_f == 15) | (gt_f == 14) | (gt_f == 13) | (gt_f == 0)))
        ...
        pred_masks_cleaned = np.delete(pred_masks_f, np.where((gt_f == 15) | (gt_f == 14) | (gt_f == 13) | (gt_f == 0)))
        ...
        d = {'class': class_names, image_fn.split("/")[-1] + '_f1_score': per_class_f1_final}
